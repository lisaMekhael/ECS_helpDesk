...
const SetRolePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const dataToSend =
        formData.role === "agent"
          ? { ...formData, role: formData.agentType }
          : formData;
      const response = await axios.put(
        `${backend_url}/setRole/${formData.targetUserId}`,
        dataToSend,
        {
          withCredentials: true,
        }
    ...
    >
      <div className="container">
        <form className="form" onSubmit={handleSubmit}>
          <label className="lb" htmlFor="targetUserId">
            Select User
          </label>
          <select
            required
          ...
          <label className="lb" htmlFor="role">
            Role
          </label>
          <select
            required
          ...
          {formData.role === "agent" && (
            <div>
              <label className="lb" htmlFor="agentType">
                Agent Type
              </label>
              <select
                required
